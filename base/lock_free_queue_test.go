...
func TestNewQueue(t *testing.T) {
	...
	fmt.Println("captity ", queue.Capaciity())
	fmt.Println("have cap ", queue.Quantity())

	_, left := queue.Put(1)
	fmt.Println("put element ", left)
	puts, left1 := queue.Put([]int{2, 3})
	fmt.Println("put element ", left1, " puts ", puts)

	ele, ok, quantity := queue.Get()
	fmt.Println("get element ", ele, " ok ", ok, " quantity ", quantity)
